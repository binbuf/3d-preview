...
| FR-05 | File reads, cache validation, parsing, normalization, texture decode/transcode, LOD construction, compatibility-host IPC, default-heap allocation, and copy recording/submission MUST not execute on the Win32 UI thread. |
...
| NFR-14 Import isolation | OpenUSD and any future compatibility importer MUST run only in the compatibility host under a zero-capability AppContainer token and process/job memory/time limits, with brokered read-only source access, no network capability, and no environment/model-selected plug-in loading. |
...
10. the AppContainer OpenUSD host passes broker/path/network denial, memory/time limit, crash, cancellation, and malformed-stage tests without compromising the viewer.
