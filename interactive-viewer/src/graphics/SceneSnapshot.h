...
struct ReadyResourceInfo {
    ...
    uint32_t lodLevel = 0;
    // Product-owned aligned allocation bytes and frame visibility, used by
    // the eviction planner. Source-range re-import and lifetime remain owned
    // by the product's model tables and deferred direct/copy fence queues.
    uint64_t approximateBytes = 0;
    uint32_t lastVisibleFrame = 0;
    float projectedError = 0;
};
