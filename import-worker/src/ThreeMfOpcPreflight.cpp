...
ThreeMfOpcError InspectThreeMfOpc(std::span<const std::byte> b, ThreeMfOpcPackage* out,const ThreeMfOpcLimits& lim,const std::function<bool()>& cancelled) {
 ...
 const bool directory=!folded.empty()&&folded.back()=='/';if(directory&&(comp||uncomp))return ThreeMfOpcError::InvalidDirectory;if(uncomp>lim.maxEntryBytes)return ThreeMfOpcError::EntryTooLarge;
 // Compare by division: ZIP64 counts are untrusted 64-bit values and
 // multiplying compressed bytes by the ratio can wrap before the range check.
 if((comp==0&&uncomp!=0)||(comp!=0&&(uncomp/comp>lim.maxExpansionRatio||(uncomp/comp==lim.maxExpansionRatio&&uncomp%comp!=0))))return ThreeMfOpcError::ExpansionRatio;
 if(uncomp>lim.maxExpandedBytes||expanded>lim.maxExpandedBytes-uncomp)return ThreeMfOpcError::AggregateTooLarge;expanded+=uncomp;
